...
## Status snapshot — updated 2026-07-15

**Phases 0–4 are built and live** (CardFoo.com / pullhit.com). **Phase 5 (Marketplace) is deferred to the *final* phase — it gates on sorting out payments (merchant-of-record / Stripe Connect / payout compliance) and isn't worth the build time until that's settled.** Near-term effort goes to deepening what's live (pricing coverage, mobile/PWA, monetization). Substantial scope was added on top of the original brief — see **§14** (2026-06 base sprint), **§14.1** (community/giveaways, multi-game image backfill, broad eBay sweeps, homepage), **§14.2** (late-June: **sealed product catalog + valuation**, a **4th game (Cyberpunk)**, **social graph/profiles**, AI-assisted sweep matching, shareable collection folders), **§14.3** (early-July: **pricing depth** — PriceCharting completed-sales + multi-year history blended into price charts, cited sealed MSRP sourcing, retailer/prerelease **stamp promos** as distinct printings, the **Compare** page, where-to-buy unified on the deals tracker), and **§14.4** (mid-July: **scanner-accuracy overhaul** name-anchored + set-code matching, **collections/folders UX redesign**, **eBay comp-quality + fetch reliability**, search **auto-correct**).
...
## 14.4 Sprint — mid-2026-07 (scanner accuracy, collections/folders UX, comp quality + reliability, search)

Layered on §14.3, same guardrails (§13). Roughly by impact:

- **Scanner accuracy overhaul** *(biggest).* Reworked `CandidateMatcher` around the insight (from reviewing real `scan_feedback`) that the AI reads the card **name** reliably but frequently misreads/fabricates the collector number and set. **Name is the anchor:** a match requires the *core* name to agree — a shared mechanic token ("ex"/"gx"/"mega") alone is **not** a match (killed the "Mega Greninja ex → Mega Charizard ex" collapse); a balanced **Dice** name score ranks an exact name above one carrying an extra word ("Mega"); number/set-code are **positive-only** (a mismatch is *not* penalised — that punished the correctly-named card) and **scaled by name quality**, so a bare number coincidence can't lift a weak partial-name match ("Energy Recycler" beats "V Guard Energy #169"). **Read the printed set CODE** ("MEW" = 151, "OP13", "BLK") in the vision pass and match `set.code` verbatim — pins the exact set where the fuzzy set *name* can't ("MEW" ≠ "151"). **Fetch fixes:** query the **raw set-coded number** verbatim (the numerator strips "OP13-098" → the card was never fetched, so a One Piece card matched a Lorcana coincidence); **name-dominant fetch ordering** + stop-word drop so a correct-but-unpopular card ("Sneezy #42") isn't crowded out of the 100-row window by a common number ("40"). **Transparency UI:** the confirm card headlines the **matched** card (not the AI's raw misread number/set), shows the read set code + "**Matched on**" chips + a low-confidence flag. **Self-healing:** a manual correction teaches the AI-free **recognition cache** (phash → correct item, heals the wrong hit) so a re-scan is instant; `scan:backfill-fingerprints` seeds it from past feedback. Result on a mixed One Piece / Lorcana / Pokémon scan: correct card across all three games. Scanning now defaults to **single** mode (live is opt-in).
- **Collections & folders UX redesign.** Folders are scoped to one collection, but that was invisible and folders were second-class. Added a sidebar **Collections tree** (each collection expands to its own folders), a dashboard **"Your collections"** hierarchy card (value + folder counts, computed from already-loaded portfolio items), dedicated **folder pages** (`/collection/folders/{id}`, breadcrumb Collections ▸ Collection ▸ Folder, folder-scoped summary + holdings, own share controls), **first-class folders** (create empty / delete → un-files cards / per-folder share), and a shared **collection + folder dropdown picker with "+" to create** used *everywhere* a card enters a collection — the add-to-collection dialog, the edit/move dialog (move to a new collection inline), and the **scan** confirm flow. Extracted a reusable `HoldingsTable`; `BuildCollectionTree` powers the shared sidebar prop.
- **eBay comp quality + fetch reliability.** Search terms rewritten to match how listings are actually titled: sealed = "**{Game} - {Product}**" (dropped the duplicated set name + noise `(CODE)`), singles = "**{Game} - {Name} - {Variant} - {Number}**", plus the eBay **Language** aspect (English/Japanese/…) and a US ship-to ZIP (`_stpos`) to kill cross-language pollution. **Fetch reliability:** **retry** on eBay's intermittent degraded/empty render (trusts only an explicit "no matches" as a real zero), a **dual-layout** HTML parser (current `s-card` + legacy `s-item`), and block detection so an anti-bot page isn't cached as "0 comps" for the TTL. **Classifier:** a sealed product stating its **pack count** ("Booster Bundle 6 Packs") is no longer rejected as a multi-quantity lot (the count is keyed to the product's own unit); Lorcana **foil** ("cold foil") modeled as a distinct printing + **2,590** foil variants generated; One Piece special-printing guards broadened (alt-art/parallel/full-art/championship/…). **Admin comp-preview modal** on the card page — run the live eBay sold search and see every candidate tagged **ingest vs reject + reason** + resolved priced state, a read-only window into exactly what a refresh would do.
- **Search auto-correct.** Google-style "**showing results for X**" auto-correction plus a "**did you mean**" suggestion on zero-result / typo browse searches (`SuggestSearch::didYouMean`, driver-aware SOUNDEX/prefix prefilter, server-authoritative original query so the banner never blanks).
- **Other.** Price **surge flag** (🔥) on cards spiking on real recent sales; **Biggest Movers** page (gainers/losers, brand/set/type filters, combobox); **cross-language** card links (JP↔EN Pokémon by set/number/variant); **JP Pokémon import** (101 sets / ~14.4k items, ~93% images); admin catalog **structure** reference + series rename/merge + subsets (Shiny Vault as a Trainer-Gallery-style subset); mobile header **hamburger** menu on non-dashboard pages; **chart watermark** (CardFoo lockup behind value + compare charts); admin stock-alerts → linked catalog-item edit; **Rip or Keep** EV page (SV Pokémon, cited pull-rate researcher); collection-modal "set total I own" quantity model.

### Updated next focus (2026-07-15)

> **Marketplace (Phase 5) is intentionally LAST** — it gates on payments (merchant-of-record / Stripe Connect / payout compliance). §15 Tier 1 #1 (**public price-history charts**) and #3 (**graded + sealed pricing**) are **done**; the **scanner** and **collections/folders** UX were deepened materially (§14.4). Pricing accuracy + coverage and the mobile/PWA gap are the near-term levers.

1. **Phase 6 — PWA + mobile bottom-tab nav.** *Still the highest-value near-term build* — the brief's mobile-first goal is only partly met (sidebar, not bottom-tab; no install/offline). Scanning is a phone-first flow, so this compounds with the §14.4 scanner work.
2. **Public "see the comps"** (§15 Tier 1 #2). The admin comp-preview is built but admin-only; surface a **public**, read-only "N sold · last sale · confidence · range" + expandable comp list (date/price/venue, sold-vs-asking) on every card/sealed page — the cheap conversion + SEO wedge.
3. **Import the promo/stamp variants** — the `stamp` facet + per-stamp comp routing are built; now **populate** the GameStop / EB Games / prerelease / staff variants (source a list + bulk/admin add) so their comps route + value instead of being pruned off the base card.
4. **Finish MSRP + PriceCharting coverage** — run `sealed:backfill-msrp` and `valuation:sweep-pricecharting` across all lines/types; resolve the remaining unmatched PriceCharting products + sealed-unmatched sets (name-match / override map).
...
6. **Early Adopter product** — $250 one-time, Guru benefits + badge + 500 monthly giveaway entries (Dodo product id staged in env). Quick monetization win once billing (Dodo) is wired end-to-end.
7. **Price alerts** (§15 Tier 2 #4) — broaden the wishlist target-price alerts to "any card/sealed below $X / on a % drop"; retention lever, reuses the sweep + notification plumbing.
8. **Phase 5 — Marketplace** *(final, payments-blocked).* First-party listings/orders/payments → first-party **sold data** → the wash-trade-detection wedge. Resume once the money rails are decided.
